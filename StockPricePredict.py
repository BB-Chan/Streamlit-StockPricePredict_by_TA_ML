...
st.sidebar.write('Select Streamlit Cloud or Local_PC to run this program,')
Local_PC = st.sidebar.checkbox('Tick if use Local_PC')
# Defining a Button
...
def get_adx(df1):
    ...
    tr1 = pd.DataFrame(df1['High'] - df1['Low'])
    tr2 = pd.DataFrame(abs(df1['High'] - df1['Close'].shift(1)))
    tr3 = pd.DataFrame(abs(df1['Low'] - df1['Close'].shift(1)))
    frames = [tr1, tr2, tr3]
